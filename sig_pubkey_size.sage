def sig_and_pubkey_size(lam, q, m, n, r, k, t, w):
    ...
    tree_inner = (t - w) * log(t / (t - w), 2).n() + wt_t - 1
    seed_tree = 3 * lam * floor(tree_inner)
    ...
    print(f"Signature Size = {sig_size.n() / 8000:.2f} kB, Public Key Size = {pubkey_size.n() / 8:.2f} B")
...
def ring_signature_size(lam, q, m, n, r, k, t, w, ell):
    ...
    print(f"Signature Size = {sig_size.n() / 8000:.2f} kB")



sig_and_pubkey_size(lam=128, q=127, m=8, n=16, r=8, k=30, t=176, w=115)
